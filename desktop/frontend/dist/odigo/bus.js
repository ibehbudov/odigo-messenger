...
(function () {
  ...
  function paintOverlay() {
    ...
    if (!el) {
      ...
      el.id = '__odbg';
      el.style.cssText = 'position:fixed;left:0;right:0;bottom:0;z-index:99999;background:rgba(0,0,0,.82);color:#0f0;font:10px/1.35 monospace;padding:3px 5px;max-height:46%;overflow:auto;white-space:pre-wrap;pointer-events:none;';
      document.body.appendChild(el);
    }
    el.textContent = 'D=' + DESKTOP + ' iframe=' + WEB_IFRAME + ' origin=' + location.origin + ' path=' + location.pathname + '\n' + DBG.slice(-8).join('\n');
  }
  function dbg(m) {
    DBG.push(m);
